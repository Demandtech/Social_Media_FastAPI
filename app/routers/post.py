...
@router.get('/user', status_code=status.HTTP_200_OK,  response_model=List[schema.PostOut])
def get_user_post(db: session = Depends(get_db),  current_user: dict = Depends(oauth2.get_current_user), limit=10):
...
def update_post(id: int, updated_post: schema.PostCreate, db: session = Depends(get_db),  current_user: dict = Depends(oauth2.get_current_user)):
    post_query = db.query(models.Post).filter(models.Post.id == id)
